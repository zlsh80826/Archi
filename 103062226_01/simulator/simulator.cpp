...
Word simulator::InstructionFetch() {
    ...
    printf("%d %llu %x %d\n", cycle, program_count / 4, instructionMemory->instruction_set[program_count / 4].value,
           registers->registerFile[31].value);
...
bool simulator::InstructionDecode(Word instruction) {
    ...
    if (type == R_type) {
        ...
        if (rd_index == 0x00000000 && funct != 0x08) {
            if (((instruction.getRt()) == (0x00000000)) && (instruction.getRd() == (0x00000000)) &&
                (instruction.getShamt() == (0x00000000) && (instruction.getFunct() == (0x00000000)))) {
        ...
        if (rt_index == 0x00000000 && (opcode == 0x08 || opcode == 0x09 || opcode == 0x23 || opcode == 0x21 ||
                                       opcode == 0x25 || opcode == 0x20 || opcode == 0x24 || opcode == 0x0f ||
                                       opcode == 0x0c || opcode == 0x0d || opcode == 0x0e || opcode == 0x0a)) {
            error->RError(cycle);
            write0 = true;
            if (opcode == 0x08 || opcode == 0x23 || opcode == 0x21 || opcode == 0x25 || opcode == 0x20 ||
                opcode == 0x24) {
                if (opcode == 0x08) {
...
void simulator::ExcuteStage(uint32_t opcode, Word* rs, Word* rt, uint32_t immediate) {
    ...
    if (opcode == 0x08) {
        ...
        if (offset % 4 == 0) {
            dataMemory->DataMemory_set[offset / 4].value =
                ((dataMemory->DataMemory_set[offset / 4].value) & (0x0000ffff)) + (temp << 16);
        } else if (offset % 2 == 0) {
            dataMemory->DataMemory_set[offset / 4].value =
                ((dataMemory->DataMemory_set[offset / 4].value) & (0xffff0000)) + temp;
        } else {
            // error
        }
    } else if (opcode == 0x28) {
        ...
        if (offset % 4 == 0) {
            dataMemory->DataMemory_set[offset / 4].value =
                ((dataMemory->DataMemory_set[offset / 4].value) & (0x00ffffff)) + temp << 24;
        } else if (offset % 4 == 1) {
            dataMemory->DataMemory_set[offset / 4].value =
                ((dataMemory->DataMemory_set[offset / 4].value) & (0xff00ffff)) + temp << 16;
        } else if (offset % 4 == 2) {
            dataMemory->DataMemory_set[offset / 4].value =
                ((dataMemory->DataMemory_set[offset / 4].value) & (0xffff00ff)) + temp << 8;
        } else if (offset % 4 == 3) {
            dataMemory->DataMemory_set[offset / 4].value =
                ((dataMemory->DataMemory_set[offset / 4].value) & (0xffffff00)) + temp;
        }
    } else if (opcode == 0x0f) {
        printf("%x\n", immediate);
...
void simulator::check_overflow(uint32_t source1, uint32_t source2, char add_sub) {
    ...
    if (add_sub == '+') {
        ...
        // printf("%x %x %x %x %x %x\n", sum, source1, source2, (sum&0x80000000), source1&0x80000000,
        // source2&0x80000000);
